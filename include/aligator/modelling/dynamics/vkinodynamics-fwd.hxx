...
VkinodynamicsFwdDynamicsTpl<Scalar>::VkinodynamicsFwdDynamicsTpl(
  ...
  nj_ = model.nv - 6;
  nf_ = int(contact_states.size()) * force_size;
...
template <typename Scalar>
void VkinodynamicsFwdDynamicsTpl<Scalar>::forward(const ConstVectorRef &x,
                                                  const ConstVectorRef &u,
                                                  BaseData &data) const {
  Data &d = static_cast<Data &>(data);
  pinocchio::DataTpl<Scalar> &pdata = d.pin_data_;
  const auto q = x.head(pin_model_.nq);
  const auto vj = u.tail(nj_);

  d.v0_.setZero();
  pinocchio::crba(pin_model_, pdata, q); // Compute Ag
  ...
  d.xdot_.template tail<6>() = d.cforces_;
}

template <typename Scalar>
void VkinodynamicsFwdDynamicsTpl<Scalar>::dForward(const ConstVectorRef &x,
                                                   const ConstVectorRef &u,
                                                   BaseData &data) const {
  Data &d = static_cast<Data &>(data);
  pinocchio::DataTpl<Scalar> &pdata = d.pin_data_;
  ...
  for (std::size_t i = 0; i < contact_states_.size(); i++) {
    ...
    if (contact_states_[i]) {
      ...
      d.Jx_.block(pin_model_.nv + 3, 0, 3, pin_model_.nv) +=
          d.Jtemp_ * (pdata.Jcom - d.fJf_.template topRows<3>());
    }
  }
  // Compute d(Ag * q_dot)/ dq
  d.v0_.template head<6>().setZero();
  d.v0_.tail(nj_) = vj;
  d.a0_.setZero();
  ...
  d.v0_.setZero();
  d.a0_.setZero();
  d.a0_.template head<6>() = d.xdot_.template head<6>();
  pinocchio::computeCentroidalDynamicsDerivatives(pin_model_, pdata, q, d.v0_,
                                                  d.a0_, d.dh_dq_, d.dhdot_dq_,
                                                  d.dhdot_dv_, d.dhdot_da_);
  ...
  for (std::size_t i = 0; i < contact_states_.size(); i++) {
    ...
    if (contact_states_[i]) {
      ...
      d.Ju_.block(pin_model_.nv, force_size_ * i_, 3, 3).noalias() +=
          Eigen::MatrixXd::Identity(3, 3);
      d.Ju_.block(pin_model_.nv + 3, force_size_ * i_, 3, 3).noalias() +=
          d.Jtemp_;
      if (force_size_ == 6) {
        d.Ju_.block(pin_model_.nv + 3, force_size_ * i_ + 3, 3, 3).noalias() +=
            Eigen::MatrixXd::Identity(3, 3);
      }
    }
  }
...
VkinodynamicsFwdDynamicsTpl<Scalar>::createData() const {
  return allocate_shared_eigen_aligned<Data>(this);
}

template <typename Scalar>
VkinodynamicsFwdDataTpl<Scalar>::VkinodynamicsFwdDataTpl(
    const VkinodynamicsFwdDynamicsTpl<Scalar> *model)
  ...
  this->Ju_
      .block(6, model->nf_, model->pin_model_.nv - 6, model->pin_model_.nv - 6)
      .setIdentity();
